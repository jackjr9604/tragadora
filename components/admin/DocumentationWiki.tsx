...
export function DocumentationWiki({ articles, canCreate, initialSection }: { articles: DocumentationSummary[]; canCreate: boolean; initialSection?: string }) {
  ...
  function selectSection(value: string | null) { setSection(value); setQuery('') }
    ...
    <div className="grid gap-6 lg:grid-cols-[250px_minmax(0,1fr)]">
      ...
      <section className="min-w-0">
        <div className="rounded-2xl bg-slate-950 p-6 text-white sm:p-8">
          <p className="text-xs font-semibold uppercase tracking-[.18em] text-amber-300">Base de conocimiento interna</p>
          <h1 className="mt-2 text-3xl font-bold">{section ?? 'Documentación de Tragadora'}</h1>
          <p className="mt-3 max-w-2xl text-slate-300">{section ? MODULE_DESCRIPTIONS[section] ?? 'Manual administrativo y operativo del módulo.' : 'Manual funcional, administrativo y operativo.'}</p>
          <label className="mt-6 flex max-w-xl items-center gap-2 rounded-lg border border-white/15 bg-white/10 px-3">
            <Search className="size-4 text-slate-300" />
            <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar en todos los artículos..." className="min-w-0 flex-1 bg-transparent py-3 text-sm text-white outline-none placeholder:text-slate-400" />
          </label>
        </div>
...
function SidebarGroup({ title, items, available, selected, onSelect }: { title: string; items: string[]; available: string[]; selected: string | null; onSelect: (value: string) => void }) {
  return <div className="mt-5"><p className="px-3 text-[10px] font-bold uppercase tracking-[.16em] text-slate-400">{title}</p><nav className="mt-1 space-y-0.5">{items.filter((item) => available.includes(item)).map((item) => <button key={item} onClick={() => onSelect(item)} className={`flex w-full items-center justify-between rounded-md px-3 py-1.5 text-left text-sm ${selected === item ? 'bg-amber-50 font-semibold text-amber-900' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-950'}`}><span>{item}</span><ChevronRight className="size-3.5" /></button>)}</nav></div>
}
...
function ModuleGrid({ title, items, articles, available, onSelect }: { title: string; items: string[]; articles: DocumentationSummary[]; available: string[]; onSelect: (value: string) => void }) {
  return <section><h2 className="text-sm font-bold uppercase tracking-[.14em] text-slate-500">{title}</h2><div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{items.filter((item) => available.includes(item)).map((item) => { const count = articles.filter((article) => article.section === item).length; return <button key={item} onClick={() => onSelect(item)} className="group flex min-h-32 flex-col rounded-xl border bg-white p-4 text-left shadow-sm transition hover:border-amber-300 hover:shadow"><div className="flex items-center justify-between"><BookOpen className="size-5 text-amber-600" /><span className="text-xs text-slate-400">{count} {count === 1 ? 'artículo' : 'artículos'}</span></div><h3 className="mt-3 font-bold text-slate-950">{item}</h3><p className="mt-1 line-clamp-2 text-sm text-slate-500">{MODULE_DESCRIPTIONS[item]}</p><span className="mt-auto pt-3 text-xs font-semibold text-amber-700">Abrir manual →</span></button> })}</div></section>
}

function ModuleHub({ articles }: { articles: DocumentationSummary[] }) {
  const groups = groupArticles(articles)
  return <div className="mt-7 space-y-7"><div className="rounded-xl border bg-white p-5 shadow-sm"><p className="text-sm leading-6 text-slate-600">Explora el manual por tarea o bloque de la pantalla. Los artículos conservan su contenido detallado y están ordenados por subsección.</p></div>{groups.map(([subsection, items]) => <section key={subsection}><h2 className="mb-3 text-sm font-bold uppercase tracking-[.12em] text-slate-500">{subsection}</h2><div className="overflow-hidden rounded-xl border bg-white shadow-sm">{items.map((article, index) => <Link key={article.id} href={`/admin/docs/${article.slug}`} className={`flex items-center justify-between gap-4 p-4 transition hover:bg-amber-50/50 ${index ? 'border-t' : ''}`}><div className="min-w-0"><h3 className="font-semibold text-slate-950">{article.title}</h3><p className="mt-1 line-clamp-2 text-sm text-slate-500">{article.excerpt || 'Sin resumen.'}</p></div><ChevronRight className="size-4 shrink-0 text-amber-600" /></Link>)}</div></section>)}</div>
}

function SearchResults({ articles, query }: { articles: DocumentationSummary[]; query: string }) {
  return <div className="mt-7"><p className="mb-3 text-sm text-slate-500">{articles.length} resultados para “{query.trim()}” en toda la documentación</p><div className="overflow-hidden rounded-xl border bg-white">{articles.map((article, index) => <Link key={article.id} href={`/admin/docs/${article.slug}`} className={`flex items-center justify-between gap-4 p-4 hover:bg-slate-50 ${index ? 'border-t' : ''}`}><div><p className="text-xs font-semibold uppercase tracking-wider text-amber-700">{article.section}{article.subsection ? ` · ${article.subsection}` : ''}</p><h2 className="mt-1 font-semibold">{article.title}</h2><p className="mt-1 line-clamp-1 text-sm text-slate-500">{article.excerpt}</p></div><ChevronRight className="size-4 shrink-0" /></Link>)}</div>{!articles.length && <div className="rounded-xl border border-dashed bg-white p-10 text-center text-slate-500">No encontramos artículos.</div>}</div>
}
