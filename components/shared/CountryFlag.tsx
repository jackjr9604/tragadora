...
export function CountryFlag({ countryCode, countryName, showName = false, size = 'md' }: Props) {
  ...
  return <span className="group/flag relative z-20 inline-flex items-center gap-2">
    <button type="button" onClick={() => setOpen((value) => !value)} onBlur={() => setOpen(false)} aria-label={`País de origen: ${name}`} aria-expanded={open} title={name} className="inline-flex h-7 min-w-8 items-center justify-center rounded-md border border-current/15 bg-white/5 px-1 outline-none focus-visible:ring-2 focus-visible:ring-amber-300/70">
      <span aria-hidden="true" className={size === 'sm' ? 'text-lg leading-none' : 'text-xl leading-none'}>{flag}</span>
      <span className="sr-only">País de origen: {name}</span>
    </button>
