...
export async function PublicPageShell({ language = 'es', children }: { language?: PublicLanguage; children: ReactNode }) {
  ...
  return <main className="tradagora-shell min-h-screen overflow-x-clip bg-[#09111f] text-slate-100"><Suspense fallback={<TickerFallback />}><AsyncTicker /></Suspense><PublicNavbar language={activeLanguage} languages={languages} /><div className="flex flex-col">{children}</div><PublicFooter language={activeLanguage} /></main>
}
