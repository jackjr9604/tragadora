...
export async function PublicPageShell({ payouts, language = 'es', children }: { payouts: HomePayout[]; language?: PublicLanguage; children: ReactNode }) {
  ...
  return <main className="tradagora-shell min-h-screen overflow-x-clip bg-[#09111f] text-slate-100"><LivePayoutTicker payouts={payouts} /><PublicNavbar language={activeLanguage} languages={languages} /><div className="flex flex-col">{children}</div><PublicFooter language={activeLanguage} /></main>
}
