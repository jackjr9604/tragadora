...
export function BrokerDirectory({ brokers, language }: { brokers: PublicBroker[]; language: string }) {
  ...
  return <div>
    ...
    <div className="mt-5 hidden lg:block"><div className="tg-data-header tg-data-grid grid grid-cols-[minmax(230px,1.2fr)_150px_minmax(190px,1fr)_150px_minmax(180px,1fr)_120px] gap-4 px-5 py-3 text-[11px] font-semibold uppercase tracking-[.14em] text-slate-500"><span>Broker</span><span>País</span><span>Mercados</span><span>Depósito mínimo</span><span>Regulación</span><span className="text-right">Acción</span></div><div className="mt-2 space-y-2">{visible.map((broker) => <BrokerRow key={broker.id} broker={broker} language={language} />)}</div></div>
...
function BrokerRow({ broker, language }: { broker: PublicBroker; language: string }) { return <article className="tg-data-grid relative grid min-h-20 grid-cols-[minmax(230px,1.2fr)_150px_minmax(190px,1fr)_150px_minmax(180px,1fr)_120px] items-center gap-4 rounded-xl border border-white/8 bg-[#111c2e]/75 px-5 py-3 transition hover:border-amber-300/25 hover:bg-[#152238]"><Identity broker={broker} /><CountryFlag countryCode={broker.countryCode} countryName={broker.country} /><Badges values={broker.markets.map(marketLabel)} /><span className="font-mono text-sm text-slate-300">{deposit(broker)}</span><span className="truncate text-sm text-slate-300">{broker.regulationSummary || 'Sin verificar'}</span><Link href={`/go/${broker.slug}?lang=${language}`} className="tg-button-gold relative z-10 px-3 py-2.5 text-sm">Visitar <ArrowUpRight className="size-4" /></Link><Link href={`/brokers/${broker.slug}?lang=${language}`} aria-label={`Ver ${broker.name}`} className="absolute inset-0 z-[5] rounded-xl" /></article> }
