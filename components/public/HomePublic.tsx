...
import type { HomeOffer, HomePayout, HomePlatform, HomeRankings, PublicCountry } from '@/lib/home-data'
...
type Props = {
  ...
  stats: { totalPaid: number | null; totalPayouts: number | null; paidToday: number | null; payoutsToday: number | null; firmsActiveToday: number | null; firmsTracked: number | null; recentUpdatedAt: string | null }
}

export function HomePublic(props: Props) {
  const { content, latestPayouts, featuredPlatforms, publicOffers, language, recommendationFirms, countries, initialCriteria, rankings, stats } = props
  const featuredOffers = publicOffers.filter((offer) => offer.isFeatured)
  const sectionNodes: Record<string, React.ReactNode> = {
    hero: <section className="home-grid"><div className="mx-auto grid max-w-7xl gap-12 px-4 py-16 sm:px-6 lg:grid-cols-[1.05fr_.95fr] lg:px-8 lg:py-24"><div><Badge>{pageValue(content, 'hero', 'badge', 'Datos para operar con criterio')}</Badge><h1 className="mt-6 text-5xl font-bold leading-[.98] tracking-[-.055em] sm:text-6xl">{pageValue(content, 'hero', 'title', 'Compara antes de poner tu capital en juego.')}</h1><p className="mt-6 max-w-2xl text-lg leading-8 text-slate-400">{pageValue(content, 'hero', 'subtitle', 'Prop firms, ofertas y pagos verificados en un mismo lugar.')}</p><div className="mt-8 flex flex-wrap gap-3"><Link href={`/prop-firms?lang=${language}`} className="button-gold">{pageValue(content, 'hero', 'primary_cta', 'Explorar Prop Firms')} <ArrowRight className="size-4" /></Link><Link href={`/payouts?lang=${language}`} className="button-outline">{pageValue(content, 'hero', 'secondary_cta', 'Ver pagos verificados')}</Link></div></div><div className="rounded-[1.75rem] border border-white/12 bg-[#111c2e]/90 p-7"><p className="font-mono text-xs uppercase text-slate-400">Actividad verificada hoy</p>{stats.paidToday === null ? <div className="mt-5"><strong className="block text-xl text-white">Sin datos recientes disponibles</strong><p className="mt-2 text-xs text-slate-500">Esto no significa que el monto pagado sea cero.</p></div> : <><strong className="mt-5 block font-mono text-4xl text-emerald-400">{money.format(stats.paidToday)}</strong><p className="mt-2 text-xs text-slate-500">{stats.payoutsToday} retiros · {stats.firmsActiveToday} firmas con actividad</p></>}{stats.recentUpdatedAt && <p className="mt-2 text-[11px] text-slate-600">Última evidencia: {new Date(stats.recentUpdatedAt).toLocaleString('es-CO')}</p>}{latestPayouts.slice(0, 3).map((payout) => <div key={payout.id} className="mt-3 flex items-center gap-3 rounded-xl bg-white/[.04] p-3">{payout.platform && <PlatformLogo platform={payout.platform} small />}<span className="min-w-0 flex-1 truncate text-sm font-semibold">{payout.platform?.name ?? 'Payout verificado'}</span><span className="font-mono text-emerald-400">{money.format(payout.amount)}</span></div>)}<Link href={`/payouts?lang=${language}`} className="mt-4 inline-flex text-sm font-semibold text-[#f7c64b]">Abrir Payout Tracker →</Link></div></div></section>,
    finder: <PropFirmFinder firms={recommendationFirms} countries={countries} language={language} initial={initialCriteria} badge={pageValue(content, 'finder', 'badge', 'Recomendador')} title={pageValue(content, 'finder', 'title', 'Encuentra la Prop Firm adecuada para ti')} subtitle={pageValue(content, 'finder', 'subtitle', 'Cuéntanos cómo operas y Tradagora compara las firmas por ti.')} cta={pageValue(content, 'finder', 'cta', 'Encontrar mis mejores opciones')} />,
    payouts_preview: <section className="border-y border-white/8 bg-[#0c1626] py-16"><SectionHeader badge={pageValue(content, 'payouts_preview', 'badge', 'Payout tracker')} title={pageValue(content, 'payouts_preview', 'title', 'Pagos reales. Evidencia rastreable.')} subtitle={pageValue(content, 'payouts_preview', 'subtitle', 'Evidencias individuales registradas por Tradagora; las métricas externas permanecen separadas en el tracker.')} href={`/payouts?lang=${language}`} cta={pageValue(content, 'payouts_preview', 'cta', 'Ver Payout Tracker')} /><div className="mx-auto mt-8 grid max-w-7xl gap-4 px-4 sm:grid-cols-3 sm:px-6 lg:px-8"><Stat label="Total registrado" value={stats.totalPaid === null ? 'Sin datos' : compactMoney.format(stats.totalPaid)} /><Stat label="Payouts" value={stats.totalPayouts === null ? 'Sin datos' : stats.totalPayouts.toLocaleString('es-CO')} /><Stat label="Firmas con evidencia" value={stats.firmsTracked === null ? 'Sin datos' : String(stats.firmsTracked)} /></div></section>,
    payout_rankings: <HomePayoutRankings rankings={rankings} />,
    featured_firms: featuredPlatforms.length ? <section className="py-20"><SectionHeader badge={pageValue(content, 'featured_firms', 'badge', 'Selección editorial')} title={pageValue(content, 'featured_firms', 'title', 'Prop Firms destacadas')} href={`/prop-firms?lang=${language}`} cta={pageValue(content, 'featured_firms', 'cta', 'Ver todas las firmas')} /><div className="mx-auto mt-8 grid max-w-7xl gap-4 px-4 md:grid-cols-3 sm:px-6 lg:px-8">{featuredPlatforms.slice(0, 6).map((platform) => <article key={platform.id} className="rounded-2xl border border-white/10 bg-[#111c2e] p-6"><div className="flex items-center gap-3"><PlatformLogo platform={platform} /><div><h3 className="font-bold">{platform.name}</h3>{platform.featuredBadge && <p className="mt-1 text-xs text-[#f7c64b]">{platform.featuredBadge}</p>}</div></div><p className="mt-5 min-h-12 text-sm text-slate-400">{platform.description || 'Consulta sus datos y condiciones.'}</p><div className="mt-5 flex gap-3"><Link href={`/prop-firms/${platform.slug}?lang=${language}`} className="inline-flex items-center gap-2 text-sm font-bold">Ver detalles</Link><Link href={`/go/${platform.slug}`} className="inline-flex items-center gap-2 text-sm font-bold text-[#f7c64b]">{platform.featuredCtaLabel || 'Visitar'} <ArrowUpRight className="size-4" /></Link></div></article>)}</div></section> : null,
    featured_offers: featuredOffers.length ? <FeaturedOffersStrip offers={featuredOffers} language={language} global /> : null,
  ...
  return <>{sections.map((section) => <div key={section.name} data-home-section={section.name}>{sectionNodes[section.name]}</div>)}{publicOffers.length ? <OfferActivityToast offers={publicOffers} /> : null}</>
}
