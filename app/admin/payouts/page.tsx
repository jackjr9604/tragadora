...
export default async function PayoutsPage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  return <main className="p-4 sm:p-6 lg:p-8"><div className="mx-auto max-w-[1500px]"><AdminDirectoryHeader title="Payouts" description="Selecciona una Prop Firm para revisar sus pagos, fuentes y métricas." count={total} action={<Link href="/admin/payouts/sources/new" className="rounded-lg bg-black px-4 py-2.5 text-sm font-medium text-white">+ Nueva fuente</Link>} /><MondoTradersStatusPanel />
    <form className="my-5 grid gap-3 rounded-xl bg-white p-3 shadow-sm md:grid-cols-[minmax(240px,1fr)_220px_180px_auto]"><input name="q" defaultValue={q} placeholder="Buscar firma..." className="rounded-lg border px-3 py-2.5" /><select name="data" defaultValue={dataFilter} className="rounded-lg border px-3"><option value="">Todos los estados</option><option value="with">Con datos</option><option value="without">Sin datos</option></select><select name="sort" defaultValue={sort} className="rounded-lg border px-3"><option value="name">Nombre A-Z</option><option value="za">Nombre Z-A</option></select><button className="rounded-lg bg-black px-4 py-2.5 text-sm text-white">Aplicar</button></form>
