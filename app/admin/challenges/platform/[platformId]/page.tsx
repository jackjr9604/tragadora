...
export default async function PlatformChallengesPage({ params }: { params: Promise<{ platformId: string }> }) {
  ...
  return <main className="min-h-screen bg-slate-100 p-8"><div className="mx-auto max-w-7xl">
    <div className="mb-8 flex flex-col justify-between gap-5 sm:flex-row sm:items-center"><div className="flex items-center gap-4"><AdminLogo media={first(platform.media)} name={platform.name} /><div><p className="text-sm text-slate-500">Gestión de Challenges</p><h1 className="text-3xl font-bold">{platform.name}</h1></div></div><div className="flex gap-3"><Link href="/admin/challenges" className="rounded-lg border bg-white px-4 py-2 text-sm font-medium">Volver</Link><AdminAction permission="challenges.create"><Link href={`/admin/challenges/new?platform=${platform.id}`} className="rounded-lg bg-black px-4 py-2 text-sm font-medium text-white">+ Nuevo Challenge</Link></AdminAction></div></div>
