...
export default async function ChallengesPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  const q = ((await searchParams).q ?? '').trim().toLocaleLowerCase('es')
  ...
  const firms = (platformsResult.data ?? []).filter((platform) => !q || platform.name.toLocaleLowerCase('es').includes(q)).map((platform) => {
  ...
  return <main className="min-h-screen bg-slate-100 p-8">
    <div className="mx-auto max-w-7xl">
      ...
      <div className="mb-5 max-w-md"><SearchInput key={q} value={q} placeholder="Buscar Prop Firm..." /></div>
      <p className="mb-3 text-xs font-medium uppercase tracking-wide text-slate-500">{firms.length} resultados</p>
