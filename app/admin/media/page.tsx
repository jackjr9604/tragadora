...
import { Check, Copy, FileIcon, ImageIcon, Upload, X } from 'lucide-react'
...
export default function MediaPage() {
  const canCreate = useAdminPermission('media.create')
  const canUpdate = useAdminPermission('media.update')
  ...
  return <main className="p-4 sm:p-6 lg:p-8"><div className="mx-auto max-w-[1500px]">
    ...
    <div className="mb-5 grid gap-3 rounded-xl bg-white p-3 shadow-sm sm:grid-cols-2 xl:grid-cols-[minmax(280px,1fr)_220px_180px_auto]"><SearchInput value={search} onChange={setSearch} placeholder="Buscar archivos..." /><select value={categoryFilter} onChange={(event) => setCategoryFilter(event.target.value)} className="rounded-lg border px-3"><option value="all">Todas las categorías</option>{categories.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}<option value="unclassified">Sin clasificar</option></select><select value={typeFilter} onChange={(event) => setTypeFilter(event.target.value as TypeFilter)} className="rounded-lg border px-3"><option value="all">Todos los tipos</option><option value="image">Imágenes</option><option value="document">Documentos</option><option value="other">Otros</option></select>{canCreate && <button type="button" onClick={() => setShowUpload(true)} className="inline-flex items-center justify-center gap-2 rounded-lg bg-black px-4 py-2.5 text-sm font-medium text-white"><Upload className="size-4" /> Subir archivo</button>}</div>
