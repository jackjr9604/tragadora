...
function AdminLinks({ links }: { links: Array<{ label: string; href: string }> }) { return <nav className="space-y-1" aria-label="Administración">{links.map(({ label, href }) => <Link key={href} href={href} className="block rounded-lg px-3 py-2 text-sm text-slate-700 hover:bg-slate-100 hover:text-black">{label}</Link>)}</nav> }
