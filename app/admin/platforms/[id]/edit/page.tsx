...
export default function EditPlatformPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [detailsResult, translationResult, mediaResult, countriesResult, availabilityResult, marketsResult, sourcesResult, payoutsResult, challengesResult, plansResult, offersResult, linksResult] = results
  ...
  return <main className="min-h-screen bg-slate-100 p-8"><div className="mx-auto max-w-6xl">
    ...
    <form onSubmit={submit} className="space-y-8 rounded-xl bg-white p-8 shadow">
      ...
      <Section title="Condiciones económicas" description="Precios, cuentas, targets y drawdown permanecen en los planes." />
    ...
    <section className="mt-8 rounded-xl bg-white p-6 shadow"><h2 className="text-xl font-semibold">MondoTraders · Métricas externas</h2><p className="mt-1 mb-6 text-sm text-slate-500">Referencia externa por periodo. Estos agregados nunca se suman con los payouts verificados por Tragadora.</p><MondoTradersMetricsPanel platformId={id} /></section>
    <section className="mt-8 rounded-xl bg-white p-6 shadow"><h2 className="text-xl font-semibold">Prop Firm Match · Payout Tracker</h2><p className="mt-1 mb-6 text-sm text-slate-500">Fallback administrativo por periodo. Cada guardado crea un snapshot nuevo y conserva el anterior como histórico.</p><PropFirmMatchSnapshotsManager platformId={id} /></section>
    <section className="mt-8 rounded-xl bg-white p-6 shadow"><h2 className="text-xl font-semibold">Payout metrics</h2><p className="mt-1 mb-6 text-sm text-slate-500">Agregados oficiales o externos. No se insertan como payouts individuales y siempre conservan su fuente.</p><PayoutMetricsManager platformId={id} /></section>
  </div></main>
}
