...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return platform ? { title: `${platform.name} Payouts | Tradagora`, description: `Historial, métricas y actividad de payouts de ${platform.name}.` } : {}
}
...
export default async function PayoutFirmPage({ params, searchParams }: Props) {
  ...
  const detail = await getPayoutFirmDetail(slug, period)
  if (!detail) notFound()
