...
type PageProps = {
  ...
  searchParams: Promise<{ lang?: string | string[]; view?: string | string[]; period?: string | string[] }>
}
  ...
  return <PublicPageShell language={language}>
    <section id="firm-hero" className="tradagora-pattern border-b border-white/8">
      ...
      <div className="grid gap-7 overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-[#152238] to-[#0d1728] p-5 shadow-[0_24px_70px_rgba(0,0,0,.18)] sm:p-7 lg:grid-cols-[1fr_auto] lg:items-center">
        <div className="flex min-w-0 items-start gap-4 sm:gap-5"><div className="rounded-2xl border border-white/10 bg-white/5 p-1"><PlatformLogo platform={platform} /></div><div className="min-w-0"><p className="font-mono text-[11px] uppercase tracking-[.22em] text-cyan-300">Prop Firm</p><h1 className="mt-1 text-3xl font-bold tracking-tight sm:text-5xl">{platform.name}</h1><div className="mt-3 flex flex-wrap items-center gap-2">{row.origin_country_code && <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs text-slate-300"><CountryFlag countryCode={row.origin_country_code} countryName={countryMap.get(row.origin_country_code)} showName size="sm" /></span>}{platform.markets.map((market) => <span key={market} className="rounded-full border border-cyan-300/20 bg-cyan-300/8 px-3 py-1 text-xs font-semibold text-cyan-200">{marketLabel(market)}</span>)}</div>{(details?.ceo_name || details?.founded_at) && <dl className="mt-5 flex flex-wrap gap-x-8 gap-y-3"><SimpleFact label="CEO" value={details?.ceo_name ?? null} /><SimpleFact label="Fundada" value={details?.founded_at ? formatFoundedDate(details.founded_at) : null} /><SimpleFact label="Operando" value={details?.founded_at ? yearsOperating(details.founded_at) : null} /></dl>}</div></div>
        <div className="flex flex-wrap items-center gap-3 lg:max-w-[310px] lg:justify-end">{platform.score !== null && <div className="rounded-xl border border-emerald-400/20 bg-emerald-400/8 px-4 py-2"><p className="text-[10px] uppercase tracking-wider text-emerald-300/70">Score</p><p className="font-mono text-2xl font-bold text-emerald-400">{platform.score}<span className="text-sm text-emerald-300/50"> / 10</span></p></div>}<Link href={ctaHref} className="inline-flex items-center gap-2 rounded-lg bg-[#f7c64b] px-5 py-3 text-sm font-bold text-slate-950 shadow-lg shadow-amber-950/20 transition hover:bg-[#ffd66c]">{ctaLabel}<ArrowUpRight className="size-4" /></Link></div>
      </div>
      </div>
    </section>
    ...
    <div className="mx-auto max-w-[1440px] px-4 sm:px-6 lg:px-8">
      ...
      {activeView === 'payouts' && <main className="py-10 lg:py-14"><ViewHeading eyebrow="Payout tracker" title="Pagos" description={`Historial y actividad de payouts de ${platform.name}.`} />{payoutDetail ? <FirmPayoutDashboard detail={payoutDetail} period={payoutPeriodKey} basePath={`/prop-firms/${platform.slug}`} language={language} embedded /> : <EmptyState title="Aún no hay actividad" description="Las métricas aparecerán cuando haya información disponible para esta firma." />}</main>}
    </div>
  </PublicPageShell>
}
...
function ViewHeading({ eyebrow, title, description }: { eyebrow: string; title: string; description: string }) { return <header><p className="tg-eyebrow">{eyebrow}</p><h2 className="mt-2 text-3xl font-bold tracking-tight">{title}</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">{description}</p></header> }
function EmptyState({ title, description }: { title: string; description: string }) { return <div className="tradagora-pattern tradagora-pattern-gold mt-7 rounded-2xl border border-dashed border-white/15 px-5 py-12 text-center"><p className="font-semibold text-slate-200">{title}</p><p className="mx-auto mt-2 max-w-md text-sm text-slate-500">{description}</p></div> }
...
function SectionLabel({ children }: { children: React.ReactNode }) { return <p className="tg-eyebrow mb-2">{children}</p> }
