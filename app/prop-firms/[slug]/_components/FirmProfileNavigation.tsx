...
export function FirmProfileNavigation({ sections, name, score, logoUrl, logoAlt, ctaHref, ctaLabel }: Props) {
  ...
  return <>
    ...
    {sections.length > 0 && <nav aria-label="Índice del perfil" className={`${showCompactHeader ? 'fixed inset-x-0 top-16' : 'sticky top-0 -mx-4'} z-40 border-y border-white/10 bg-[#0a1321]/95 px-4 py-2 backdrop-blur lg:hidden`}>
      <div className="flex gap-1 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">{sections.map((section) => <button key={section.id} type="button" onClick={() => navigate(section.id)} className={`shrink-0 rounded-full px-3 py-2 text-xs font-semibold transition ${activeSection === section.id ? 'bg-cyan-300 text-slate-950' : 'text-slate-400 hover:bg-white/5 hover:text-white'}`}>{section.label}</button>)}</div>
    </nav>}

    {sections.length > 0 && <aside className="hidden lg:sticky lg:top-24 lg:block lg:self-start"><nav aria-label="Índice del perfil" className="space-y-1 rounded-xl border border-white/8 bg-[#101a2c]/70 p-2">{sections.map((section) => <button key={section.id} type="button" onClick={() => navigate(section.id)} className={`block w-full rounded-lg px-3 py-2.5 text-left text-sm transition ${activeSection === section.id ? 'bg-cyan-300/10 font-semibold text-cyan-200 shadow-[inset_3px_0_0_#67e8f9]' : 'text-slate-500 hover:bg-white/5 hover:text-slate-200'}`}>{section.label}</button>)}</nav></aside>}
  </>
}
