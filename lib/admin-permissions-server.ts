...
type Access = { userId: string | null; role: AdminRole | null; permissions: Set<PermissionKey> }

export async function getCurrentAdminPermissions(): Promise<Access> {
  const session = await createClient()
  const { data: { user }, error: authError } = await session.auth.getUser()
  if (authError || !user) return { userId: null, role: null, permissions: new Set() }

  const admin = createAdminClient()
  const profile = await admin.from('profiles').select('role').eq('id', user.id).maybeSingle()
  if (profile.error || !profile.data) return { userId: user.id, role: null, permissions: new Set() }
  const role = profile.data.role as AdminRole
  if (role === 'super_admin') return { userId: user.id, role, permissions: new Set(PERMISSION_KEYS) }
  ...
  if (preset.error || overrides.error) return { userId: user.id, role, permissions: new Set() }
